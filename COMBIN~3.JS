 ...
 *
 * v9.26 (2026-05-27): REMOVE rect.top < 70 false positive from safeClick.
 *   plomastermind7 diagnostic (ses-20260527-t7-6091d8) revealed the v9.21
 *   fast-path block_header click was refused with reason 'rect_top_in_nav_band:64'
 *   on a legitimate block at y=64. Cascade: v9.21 bail -> legacy 4-click stabilize
 *   replay -> trainer HTTP 429 -> quota_exceeded -> 0 cards walked, 16 captures
 *   burned for nothing. The rect.top check was structurally redundant with
 *   Layer 1 CSS pointer-events + Layer 2 ancestor + forbidden-text + trainingMode
 *   checks. Removing it lets v9.21 fast path re-engage on terminals where legit
 *   content lives near the top of the viewport.
 *
 * v9.25 (2026-05-27): LAYERED CLICK SAFETY -- 6 defenses against the
 *   train-button misfire bug (plomastermind4 + plomastermind4-retry incidents).
 *   ROOT CAUSE FOUND: reopenTurnModalCheapOrFull was calling reopenChipModal
 *   UNCONDITIONALLY after stabilizeBackToTerminal -- but the v9.21 fast path
 *   already opens the picker as part of stabilize. The second chip.click()
 *   was unguarded and could mis-target a nav-region element on certain DOM
 *   renders (the chip search uses /cursor-pointer/.test + /rounded-md/.test
 *   + a "Turn[2-9TJQKA]" regex, which is broad enough to occasionally match
 *   the wrong element when the picker dialog reshapes layout).
 *
 *   v9.25 fixes:
 *     Layer 1: CSS body.cron-scrape-active disables pointer-events on
 *              nav/header/[role=banner] -- physical click block during session.
 *     Layer 2: safeClick(el, opts) wrapper: validates not-detached, not in
 *              nav region, text not in forbidden list (Train/Study/etc),
 *              rect.top >= 70px; throws on training-mode entered.
 *     Layer 3: reopenChipModal SKIPS if modalKind() === kind -- the actual
 *              root-cause fix.
 *     Layer 4: (incremental) selected click sites migrated to safeClick.
 *     Layer 5: window.__msPickerOpens / __msPickerCloses counters; persisted
 *              into rec.picker_opens / rec.picker_closes by builder.
 *     Layer 6: (in cron prompts) explicit Train button .setAttribute('disabled')
 *              at pre-flight (belt-and-suspenders).
 *
 *   Nav guard auto-deactivates when __msBuildSessionRecord is called (session end).
 *
 * v9.24 (2026-05-27): TRAINING-MODE DETECTION + tripwires + zip-count validation.
 *   Incident driver: plo5plomastermind4 (2026-05-27 13:31 UTC) -- the trainer's
 *   "Train" button in the top nav (positioned RIGHT ABOVE the first turn action
 *   block) got accidentally clicked during the walk. Page silently switched into
 *   /setup?...&trainingMode=single. Helpers stayed loaded but all subsequent
 *   walks were bogus -- Th was "skipped (no real content; 4 match errors)" and
 *   Qd ended up needing user-flagged invalidation. URL had to be hand-replaced
 *   to recover, but refreshing wipes the bundle, so the session was lost.
 *
 *   v9.24 fixes:
 *     - urlTrainingMode() detects /setup pathname OR trainingMode= query param.
 *     - clickActionAndWait + clickHeaderAndWait: pre-click refuse + post-click
 *       throw + emergency-stop if training mode is observed.
 *     - __scrapeMultiStreet pre-flight: refuse to start in training mode.
 *     - clickActionAndWait + clickHeaderAndWait: DOM-staleness check (return
 *       failure if the element is no longer in document).
 *     - __msBuildSessionRecord: emits warning if cards_walked < cards_planned;
 *       sets exit_reason = 'training_mode_entered' when the tripwire fired.
 *
 * v9.23 (2026-05-27): flop_terminal_filter accepts array/string AND function;
 *   record-builder backfills tree/flop/session_id from cfg + W.url* when
 *   result.tree/flop are missing (e.g. early abort). Incident driver:
 *   plo5plomastermind1 cron passed flop_terminal_filter as ["C-C"] (array
 *   form) and crashed at 4s with "is not a function"; session_record then
 *   wrote as 'ses-unknown-<ts>.json' with null tree/flop after manual
 *   workaround. Both gaps closed in v9.23. Bundle is now forgiving --
 *   array, string, or function form all work for flop_terminal_filter.
 *
 * v9.22 (2026-05-27): DELETE v9.19 anchor + defensive picker-close.
 *   - v9.19 anchor click block in walker.dfsStreet REMOVED. Confirmed 100%
 *     failure rate on two different turn-All-in terminals (every attempt
 *     was either OOB or URL mismatch). v9.21 fast path does all recovery
 *     work cleanly. The diagnostic snapshot __msFirstTurnBlockIndex in
 *     reopenChipModal is kept (cheap, useful for future debugging).
 *   - pickCardCommit now verifies the picker DIALOG is gone after card
 *     pick. If lingering, force-close once and log. Never throws. Applies
 *     to both non-alias and alias return paths.
 *   - New trace points: pickCardCommit_dialog_lingered,
 *     pickCardCommit_dialog_closed_defensive, pickCardCommit_dialog_close_failed.
 *   - DELETED trace points: pre_v919_anchor_check, pre_v919_anchor_click_player_*,
 *     post_v919_anchor_click_*.
 *   - DELETED stabilize_log kinds: allin_anchor_click_success,
 *     allin_anchor_click_url_mismatch, allin_anchor_block_oob, allin_anchor_no_ftb_idx.
 *
 * v9.21 (2026-05-27): TURN-CARD CLICK FAST PATH for stabilizeBackToTerminal.
 *   Probe (2026-05-27) confirmed at R66-R75-C-A&turn=Qh: trainer reports
 *   chosen=false on the turn-action block (chosen marker is cleared for
 *   actions leading to terminal states). Existing heuristic picked
 *   BB Call (flop terminal) and clicking it over-popped URL two levels.
 *   v9.21 fix: trigger=target ends -C/-F/-A, urlTurn set, urlNode extends
 *   target. Two-click fast path: (1) blocks[chain_len].headerEl ->
 *   urlNode=target, turn preserved; (2) div.relative.cursor-pointer
 *   -> modalKind='turn'. Verified live. Zero range/url calls vs ~7
 *   for full replay. Bails to heuristic on any failure (no regression).
 *   _msTraceLog enhanced: chosen_idxs + active_idx from same readBlocks
 *   call. Optional `extra` arg for click context.
 *   New trace points: pre_stabilize_via_turn_card_attempt,
 *   post_stabilize_via_turn_card_block_click,
 *   post_stabilize_via_turn_card_picker_open,
 *   post_stabilize_via_turn_card_success,
 *   post_stabilize_via_turn_card_bailed.
 *   New stabilize_log kinds: stabilize_via_turn_card_success,
 *   stabilize_via_turn_card_bailed.
 *
 * v9.19 (2026-05-26): REVERT v9.18 Phase B + C + add Option 1 anchor.
 *   - v9.18 Phase B + C reverted in scrape_multistreet.js (no real cost
 *     savings; both ultimately called replayFromFlopRoot + openModalByCloser
 *     — same as legacy path).
 *   - NEW (in multi_street_walker.js): dfsStreet clicks the first-turn-block
 *     header IMMEDIATELY after All-in record, anchoring URL to flop terminal
 *     BEFORE the trainer auto-collapses the chain view (~5-10s observed
 *     window). This is the actual fix for the stabilize.truly_stuck=N
 *     pattern on -A terminals.
 *   - Diagnostics KEPT: __msFirstTurnBlockIndex, __msLastActionClicked.
 *     New stabilize_log kinds: allin_anchor_click_{success,url_mismatch},
 *     allin_anchor_block_oob, allin_anchor_no_ftb_idx, first_turn_block_visible,
 *     allin_collapse_detected.
 *   - 'allin-fast' slot in reopen_stats init kept (cosmetic).
 *
 * v9.18 (2026-05-26): All-in turn-collapse handling. Two additive fixes:
 *   (Phase B in scrape_multistreet.js + multi_street_walker.js)
 *     reopenChipModal snapshots __msFirstTurnBlockIndex when turn modal opens;
 *     stabilizeBackToTerminal uses that index as PRIMARY back-nav target
 *     (one click → URL at flop terminal). Heuristic loop is fallback.
 *   (Phase C in scrape_multistreet.js)
 *     reopenTurnModalCheapOrFull adds a branch BETWEEN cheap and full replay,
 *     gated on lastClick.code==="A" + urlTurn===null + noModal: flop-chain
 *     replay (~5 captures) instead of replay-from-root (~10 captures).
 *   New globals: __msFirstTurnBlockIndex, __msLastActionClicked.
 *   New stabilize_log kinds: attempt_first_turn_block, first_turn_block_*,
 *     allin_collapse_fast_path_*.
 *   reopen_stats init now includes 'allin-fast' slot.
 *
...
(function installMultiStreetWalker() {
  ...
  async function partialWalkOnTerminal(currentTerminal, opts) {
    // v9.26.6 (2026-05-29): ledger-driven candidate pool — MERGED onto v9.26
    //   (May 27) base preserving full v9.18-v9.26 fix stack.
    //   - opts.eligibility[currentTerminal] supplies {candidate_pool, auto_aliased, dim_dom}
    //     from the workload JSON. candidate_pool is pre-filtered at emit time;
    //     auto_aliased and dim_dom ride along as belt-and-suspenders insurance.
    //   - opts.excludeCards = FULL set of full-run cards this session.
    //   - __msPartialWalkedThisSession soft-exclude prevents 2nd-of-2 partials
    //     re-picking the 1st partial's card (stealth 1-2x).
    //   - nActions reads partial_walk_actions_range from human_like cfg.
    //   - Missing opts.eligibility -> legacy live-DOM status filter (no crash).
    opts = opts || {};
    try {
      if (modalKind() !== 'turn') return 0;

      // v9.26.6: build candidate pool from ledger eligibility, legacy fallback.
      const elig = (opts.eligibility || {})[currentTerminal] || null;
      const hardExclude = new Set([
        ...((opts.excludeCards || [])),
        ...((elig && elig.auto_aliased) || []),
        ...((elig && elig.dim_dom)      || []),
      ]);
      ...
      if (usedFallback) {
        window.__msPartialWalkFallbackUsed = (window.__msPartialWalkFallbackUsed || 0) + 1;
      }
      try {
      ...
      // v9.26.6: profile-driven via partial_walk_actions_range, default [1,2]
      ...
      // v9.26.6: record committed card in session-local soft-exclude
  ...
  async function humanCategoryExploration(k) {
    ...
    if (!panel) {
      // v9.26.6 telemetry: count silent "panel missing" failures
      window.__msCatBurstPanelMissing = (window.__msCatBurstPanelMissing || 0) + 1;
    ...
    if (!chevrons.length) {
      // v9.26.6 telemetry: count silent "no chevrons in panel" failures
      window.__msCatBurstNoChevrons = (window.__msCatBurstNoChevrons || 0) + 1;
      return 0;
    }
    ...
    // v9.26.6: profile-driven gap + telemetry (click attempted/succeeded counters).
    const [_cgLo, _cgHi] = _hRange('category_gap_ms_range', 1000, 3000);
    window.__msCatBurstClicksAttempted = window.__msCatBurstClicksAttempted || 0;
    window.__msCatBurstClicksSucceeded = window.__msCatBurstClicksSucceeded || 0;
    for (let i = 0; i < picks.length; i++) {
      window.__msCatBurstClicksAttempted++;
      try {
        picks[i].click();
        window.__msCatBurstClicksSucceeded++;
      } catch (_) {}
      if (i < picks.length - 1) {
        await sleep(_rint(_cgLo, _cgHi));
      }
    }
    // v9.26.6: profile-driven final settle + actual-elapsed telemetry
    const [_csLo, _csHi] = _hRange('category_settle_ms_range', 5000, 15000);
    const _settleStart = Date.now();
    await sleep(_rint(_csLo, _csHi));
    window.__msCatBurstSettleMsTotal = (window.__msCatBurstSettleMsTotal || 0) + (Date.now() - _settleStart);
    // Stats
  ...
  //   state at every code boundary point. Pure observation, no behavior change.
  //   Data persists into session_record by __msBuildSessionRecord (v9.20).
  // v9.21 (2026-05-27): Enhanced - reuses the same readBlocks() call to also
  //   extract chosen_idxs (compact CSV of block indices with any chosen action)
  //   and active_idx (index of isActive=true block, or -1). Adds optional
  //   `extra` arg for click-site context. Zero new DOM cost.
  function _msTraceLog(label, extra) {
    try {
      ...
      let n_blocks = -1, chosen_idxs = '', active_idx = -1;
      try {
        const blocks = readBlocks();
        n_blocks = blocks.length;
        const c = [];
        for (let i = 0; i < blocks.length; i++) {
          if (blocks[i].actions && blocks[i].actions.some(a => a.chosen)) c.push(i);
          if (blocks[i].isActive) active_idx = i;
        }
        chosen_idxs = c.join(',');
      } catch (_) {}
      const _entry = {
      ...
      if (extra) {
        try { Object.assign(_entry, extra); } catch (_) {}
      }
      prog.url_trace.push(_entry);
  ...
  // v9.24 (2026-05-27): training-mode detector. The trainer's "Train" button
  //   in the top nav (positioned right above the first turn action block on
  //   most boards) can be accidentally hit by a misfired click and silently
  //   switches the page into setup/training mode. URL signature:
  //     - location.pathname === '/setup' (training-mode setup page), OR
  //     - searchParams has 'trainingMode' key (any value).
  //   When this fires mid-session, ALL subsequent walks are bogus -- the
  //   helpers are still loaded but the trainer's state machine is in a
  //   different mode. Detect + abort immediately.
  const urlTrainingMode = () => {
    try {
      const u = urlObj();
      if (u.pathname && u.pathname.indexOf('/setup') === 0) return 'setup_path';
      if (u.searchParams.get('trainingMode')) return 'training_param';
      return null;
    } catch (_) { return null; }
  };

  // v9.25 (2026-05-27): LAYER 1 -- inject CSS that disables pointer-events
  //   on the top nav region during a scrape. Even if a stale click handle
  //   somehow points to a nav button (Train, Study, etc.), the physical
  //   click cannot register. Triggered by adding/removing the body class
  //   'cron-scrape-active' which __scrapeMultiStreet toggles.
  function _installNavGuardCSS() {
    try {
      if (document.getElementById('ms-nav-guard-style')) return;
      const style = document.createElement('style');
      style.id = 'ms-nav-guard-style';
      style.textContent = [
        '/* v9.25 nav guard: physical click-block during scrape */',
        'body.cron-scrape-active nav,',
        'body.cron-scrape-active header,',
        'body.cron-scrape-active [role="banner"] {',
        '  pointer-events: none !important;',
        '}',
        '/* visual hint that nav is locked */',
        'body.cron-scrape-active nav a,',
        'body.cron-scrape-active nav button {',
  ...
  function _activateNavGuard() {
    try { _installNavGuardCSS(); document.body.classList.add('cron-scrape-active'); } catch (_) {}
  }
  function _deactivateNavGuard() {
    try { document.body.classList.remove('cron-scrape-active'); } catch (_) {}
  }
  // Install the stylesheet immediately on bundle load (idempotent), but don't
  // activate the class until a scrape actually starts.
  _installNavGuardCSS();

  // v9.25 (2026-05-27): LAYER 2 -- safeClick(el, opts). Universal wrapper.
  //   Validates the element + region before clicking, then re-checks
  //   trainingMode after the click. ALL click sites in the bundle should
  //   migrate to this wrapper. Skipping validation is allowed only for
  //   close-modal-X (which lives inside a dialog) via opts.allowInDialog.
  const _FORBIDDEN_TEXTS = new Set(['Train','Study','Dashboard','Videos','Favorites','4-Card','5-Card']);
  function _navGuardCheck(el) {
    if (!el) return { ok: false, reason: 'el_null' };
    if (!document.contains(el)) return { ok: false, reason: 'el_detached_from_dom' };
    try {
      if (el.closest && el.closest('nav, header, [role="banner"]')) {
        return { ok: false, reason: 'el_in_nav_or_header' };
      }
    } catch (_) {}
    try {
      const txt = (el.textContent || '').trim();
      if (_FORBIDDEN_TEXTS.has(txt)) {
        return { ok: false, reason: 'forbidden_text:' + txt };
      }
    } catch (_) {}
    try {
      const rect = el.getBoundingClientRect();
      // v9.26 (2026-05-27): REMOVED rect.top < 70 check. It was false-positive
      //   on legitimate block headers at y=64 (plomastermind7 diagnostic showed
      //   reason 'rect_top_in_nav_band:64' on the v9.21 fast-path block click,
      //   cascading into legacy replay + quota_exceeded). The Layer 1 CSS
      //   pointer-events guard on nav/header/[role="banner"] + Layer 2 ancestor
      //   check (el.closest('nav,header,[role=banner]')) + forbidden text-list
      //   + trainingMode tripwire already provide complete coverage. Position
      //   check was structurally redundant and only added false-positives on
      //   pages where legitimate content renders near the top.
      // Keep zero-size check (catches detached/hidden elements).
      if (rect.width === 0 && rect.height === 0) return { ok: false, reason: 'el_zero_sized' };
    } catch (_) {}
    return { ok: true };
  }
  async function safeClick(el, opts) {
    ...
    // Pre-click training-mode check
    if (urlTrainingMode()) {
      window.__msEmergencyStop = true;
      window.__msAborted = true;
      try { window.__msTraceLog && window.__msTraceLog('safeClick_refused_trainingMode_pre', { purpose, mode: urlTrainingMode() }); } catch (_) {}
      throw new Error('safeClick refused: trainingMode=' + urlTrainingMode() + ' (purpose=' + purpose + ')');
    }
    // Region / staleness / text validation
    const chk = _navGuardCheck(el);
    if (!chk.ok && !opts.bypassRegionGuard) {
      try { window.__msTraceLog && window.__msTraceLog('safeClick_refused', { purpose, reason: chk.reason }); } catch (_) {}
    ...
    try {
      ...
      try { window.__msTraceLog && window.__msTraceLog('safeClick_threw', { purpose, error: e && e.message }); } catch (_) {}
      return { success: false, reason: 'click_threw:' + (e && e.message) };
    }
    // Post-click training-mode check
    if (urlTrainingMode()) {
      window.__msEmergencyStop = true;
      window.__msAborted = true;
      try { window.__msTraceLog && window.__msTraceLog('safeClick_detected_trainingMode_post', { purpose, mode: urlTrainingMode() }); } catch (_) {}
      throw new Error('safeClick post-click tripwire: trainingMode=' + urlTrainingMode() + ' AFTER ' + purpose + ' click -- session aborted');
    }
    return { success: true };
  }
  ...
  async function closeModalX() {
    ...
    btn.click();
    await sleep(700);
    const closed = modalKind() === null;
    // v9.25: increment picker-close counter
    try { if (closed) window.__msPickerCloses = (window.__msPickerCloses||0) + 1; } catch (_) {}
    return closed;
  }
  async function reopenChipModal(kind) {
    // v9.25 (2026-05-27): SKIP if picker is already open for this kind.
    //   ROOT CAUSE of the double-click train-misfire incident
    //   (plomastermind4 2026-05-27): reopenTurnModalCheapOrFull called
    //   stabilizeBackToTerminal (which v9.21+ already opens the picker via
    //   turnCardEl.click()), then UNCONDITIONALLY called reopenChipModal
    //   which did a SECOND chip.click() with no skip-if-open. That second
    //   click was unguarded and could hit a misidentified element including
    //   the top-nav "Train" button on layouts where the regex matched.
    //   Fix: bail early if modal is already at the requested kind.
    if (modalKind() === kind) {
      try { window.__msTraceLog && window.__msTraceLog('reopenChipModal_skipped_already_open', { kind }); } catch (_) {}
      return;
    }
    ...
    // v9.25 LAYER 4: route chip click through safeClick (Train button has been
    //   observed to match this regex on some DOM renders; safeClick refuses it)
    const _scRes = await safeClick(chip, { purpose: 'reopenChipModal:' + kind });
    if (!_scRes.success) {
      try { window.__msTraceLog && window.__msTraceLog('reopenChipModal_safeClick_refused', { kind, reason: _scRes.reason }); } catch (_) {}
      throw new Error('reopenChipModal safeClick refused: ' + _scRes.reason);
    }
    const t0 = Date.now();
    while (Date.now() - t0 < 5000 && modalKind() !== kind) await sleep(100);
    await sleep(200);
    // v9.25: increment picker-open counter
    try { if (modalKind() === kind) window.__msPickerOpens = (window.__msPickerOpens||0) + 1; } catch (_) {}
  ...
  async function clickActionAndWait(actionEl, expectedNode = null, timeoutMs = 4000) {
    // v9.24 (2026-05-27): training-mode tripwire. Refuse to click if we're
    //   already in training mode (set abort flag + throw). Also re-check
    //   AFTER click — if the click somehow entered training mode (Train
    //   button misfire), emergency-stop the session.
    if (urlTrainingMode()) {
      window.__msEmergencyStop = true;
      window.__msAborted = true;
      try { window.__msTraceLog && window.__msTraceLog('training_mode_detected_pre_clickAction', { mode: urlTrainingMode(), url: location.href }); } catch (_) {}
      throw new Error('v9.24 tripwire: trainingMode=' + urlTrainingMode() + ' before clickAction -- session aborted');
    }
    // v9.24: validate click target is still in DOM
    if (!actionEl || !document.contains(actionEl)) {
      try { window.__msTraceLog && window.__msTraceLog('click_target_stale_or_missing', { kind: 'action' }); } catch (_) {}
      return { success: false, after: urlNode(), error: 'stale_or_missing_action_el' };
    }
    ...
    actionEl.click();
    // v9.24 (2026-05-27): post-click training-mode tripwire. If this click
    //   silently entered training mode (Train button misfire from a misaligned
    //   coordinate or DOM-shift race), emergency-stop the session immediately.
    try {
      if (urlTrainingMode()) {
        window.__msEmergencyStop = true;
        window.__msAborted = true;
        try { window.__msTraceLog && window.__msTraceLog('training_mode_detected_post_clickAction', { mode: urlTrainingMode(), url: location.href }); } catch (_) {}
        throw new Error('v9.24 tripwire: trainingMode=' + urlTrainingMode() + ' AFTER actionEl.click() -- Train button likely hit; session aborted');
      }
    } catch (e) {
      if (String(e.message).indexOf('v9.24 tripwire') === 0) throw e;
    }
    const t0 = Date.now();
    let succeeded = false;
  ...
  async function clickHeaderAndWait(headerEl, expectedNode, timeoutMs = 4000) {
    // v9.24 (2026-05-27): training-mode tripwire (pre-click) + DOM-staleness check.
    if (urlTrainingMode()) {
      window.__msEmergencyStop = true;
      window.__msAborted = true;
      try { window.__msTraceLog && window.__msTraceLog('training_mode_detected_pre_clickHeader', { mode: urlTrainingMode(), url: location.href }); } catch (_) {}
      throw new Error('v9.24 tripwire: trainingMode=' + urlTrainingMode() + ' before clickHeader -- session aborted');
    }
    if (!headerEl || !document.contains(headerEl)) {
      try { window.__msTraceLog && window.__msTraceLog('click_target_stale_or_missing', { kind: 'header' }); } catch (_) {}
      return { success: false, after: urlNode(), error: 'stale_or_missing_header_el' };
    }
  ...
  async function clickCardAndWait(cellEl, kind, timeoutMs = 5000) {
    ...
    // v9.25 LAYER 4: route picker cell pick through safeClick
    const _scRes = await safeClick(cellEl, { purpose: 'pickCard:' + kind });
    if (!_scRes.success) {
      try { window.__msTraceLog && window.__msTraceLog('clickCardAndWait_safeClick_refused', { kind, reason: _scRes.reason }); } catch (_) {}
      throw new Error('clickCardAndWait safeClick refused: ' + _scRes.reason);
    }
    const t0 = Date.now();
    while (Date.now() - t0 < timeoutMs) {
      await sleep(100);
  ...
  async function dfsStreet(state, walkResult, opts = {}) {
    ...
    while (iterSafety++ < 12) {
      ...
      _msTraceLog('post_clickAction_' + pick.label + '_expect_' + childPath, { action_label: pick.label, action_code: pick.code, expect: childPath, success: clickResult.success, after_observed: clickResult.after });
      ...
      if (pickIsAllIn) {
        ...
        // v9.22 (2026-05-27): v9.19 anchor click DELETED.
        //   Two sessions on two different turn-All-in terminals showed 100%
        //   failure: anchor click either OOB (no first-turn-block snapshot)
        //   or URL mismatch (clicking the block didn't move URL to state.node).
        //   v9.21 fast path in stabilizeBackToTerminal does all the actual
        //   recovery work cleanly. The v9.19 attempt was pure cost + extra
        //   trainer load. Diagnostic stays: __msFirstTurnBlockIndex is still
        //   snapshotted in reopenChipModal (cheap, useful for future debugging).
      } else {
      ...
      _msTraceLog('pre_per_level_backout_click_player_' + (myBlockNow.player || '?'), { click_idx: myIndex, click_player: myBlockNow.player || null, expect: state.node });
      const headerResult = await clickHeaderAndWait(myBlockNow.headerEl, state.node);
      _msTraceLog('post_per_level_backout_click_' + (headerResult.success ? 'ok' : 'mismatch') + '_after_' + headerResult.after, { success: headerResult.success, expect: state.node, after_observed: headerResult.after });
  ...
  async function pickCardCommit(card, kind) {
    ...
    if (!isAlias) {
      ...
      // v9.22 (2026-05-27): defensive picker-close verification.
      //   pickCardCommit's contract is "card picked, walker can proceed".
      //   waitForActionPanelStable proves clicks will work, but it doesn't
      //   guarantee the picker DIALOG element is gone -- if the trainer
      //   leaves it lingering (rare race / different build), human-noise
      //   clicks could land in it. Close defensively; never throw.
      try {
        const dlg = document.querySelector('dialog.modal-root[open]');
        if (dlg) {
          if (window.__msTraceLog) window.__msTraceLog('pickCardCommit_dialog_lingered', { kind, card, path: 'non_alias' });
          await closeModalX();
          await sleep(400);
          const stillOpen = !!document.querySelector('dialog.modal-root[open]');
          if (window.__msTraceLog) window.__msTraceLog(stillOpen ? 'pickCardCommit_dialog_close_failed' : 'pickCardCommit_dialog_closed_defensive', { kind, card, path: 'non_alias' });
        }
      } catch (_) {}
      // v9.25: count this pickCardCommit as one picker close
      try { window.__msPickerCloses = (window.__msPickerCloses||0) + 1; } catch (_) {}
    ...
    // v9.22 (2026-05-27): defensive picker-close verification (alias path).
    try {
      const dlg = document.querySelector('dialog.modal-root[open]');
      if (dlg) {
        if (window.__msTraceLog) window.__msTraceLog('pickCardCommit_dialog_lingered', { kind, card, path: 'alias', canonical: actualForKind });
        await closeModalX();
        await sleep(400);
        const stillOpen = !!document.querySelector('dialog.modal-root[open]');
        if (window.__msTraceLog) window.__msTraceLog(stillOpen ? 'pickCardCommit_dialog_close_failed' : 'pickCardCommit_dialog_closed_defensive', { kind, card, path: 'alias', canonical: actualForKind });
      }
    } catch (_) {}
    // v9.25: count alias path as one picker close (alias-recovery picker is already counted by reopenChipModal)
    try { window.__msPickerCloses = (window.__msPickerCloses||0) + 1; } catch (_) {}
  ...
  window.__W = {
    ...
    urlObj, urlNode, urlTurn, urlRiver, urlSuitMap, urlTree, urlFlop, urlTrainingMode,
    safeClick, activateNavGuard: _activateNavGuard, deactivateNavGuard: _deactivateNavGuard,
    sleep,
  ...
  window.__msV926WalkerInstalled = true; // 2026-05-29 v9.26.6: ledger-driven partial-walk pool + hoisted category timing + cat-burst telemetry
  return 'multi-street walker installed (window.__W) [v9.22 v9.19 anchor click deleted (was 100% failing); tier 2 noise: categories-tab activate + partial-browse; phase 7 safety detect + emergency stop emit; phase 4 human-like noise (long pauses + hover bursts + category exploration); v18 all-in no-descend + back-out chain collapse; v15 random 3-5s inter-node wait; v13 plomm envelope support; v11 dynamic bet sizings + 1/5 pot static]';
})();
...
(function installMSOrchestrator() {
  ...
  async function openModalByCloser(closerLabel) {
    ...
    // v9.25 LAYER 4: openModalByCloser uses safeClick via W
    const _scRes = W.safeClick ? await W.safeClick(ce, { purpose: 'openModalByCloser:' + closerLabel }) : { success: true };
    if (!_scRes.success) {
      try { window.__msTraceLog && window.__msTraceLog('openModalByCloser_safeClick_refused', { closer: closerLabel, reason: _scRes.reason }); } catch (_) {}
      return false;
    }
    let t0 = Date.now();
  ...
  async function stabilizeBackToTerminal(ftTerminalNode) {
    // v9.20: URL trace instrumentation at entry + each click attempt.
    ...
    // v9.21 (2026-05-27): TURN-CARD CLICK FAST PATH.
    //   When the walker has just finished a turn segment and we need to
    //   navigate back to the flop terminal for the next turn card, the
    //   trainer's chosen-marker on the turn-action block is CLEARED
    //   (because the chosen action led to a terminal state). The existing
    //   "last block with chosen action" heuristic then picks the
    //   flop-terminal block (e.g. BB Call at R66-R75-C) and clicking
    //   ITS header navigates URL to "before BB Call" = R66-R75, dropping
    //   both -C and the turn parameter (2-level over-pop, 100% repro).
    //
    //   Fix verified live on 2026-05-27 at R66-R75-C-A&turn=Qh:
    //     (1) click blocks[ftTerminalNode.split('-').length].headerEl
    //         -> urlNode becomes ftTerminalNode, urlTurn preserved
    //     (2) click div.relative.cursor-pointer on board (the turn card)
    //         -> modalKind becomes 'turn' (picker dialog open)
    //
    //   End state matches ensureTurnModalAtTerminal's replay-path end state
    //   exactly. Zero range/url calls vs ~7 for full replay.
    //   On any failure: bail to v9.18 Phase B / heuristic below. No regression.
    {
      const curUrl = W.urlNode();
      const curTurn = W.urlTurn();
      const targetIsFlopTerm = /-[CFA]$/.test(ftTerminalNode);
      const trigger = targetIsFlopTerm && curTurn && curUrl && curUrl.startsWith(ftTerminalNode + '-');
      if (trigger) {
        const chainLen = ftTerminalNode.split('-').length;
        const blocks0 = W.readBlocks();
        if (window.__msTraceLog) window.__msTraceLog('pre_stabilize_via_turn_card_attempt', {
          target: ftTerminalNode, chain_len: chainLen,
          urlNode_before: curUrl, urlTurn_before: curTurn,
          n_blocks_at_entry: blocks0.length,
        });
        if (blocks0.length > chainLen && blocks0[chainLen] && blocks0[chainLen].headerEl) {
          const targetBlock = blocks0[chainLen];
          const targetPlayer = targetBlock.player || null;
          try {
            const _sc = W.safeClick ? await W.safeClick(targetBlock.headerEl, { purpose: 'v9.21_block_header_click' }) : null;
            if (_sc && !_sc.success) {
              if (window.__msTraceLog) window.__msTraceLog('v9.21_block_header_safeClick_refused', { reason: _sc.reason });
              // bail out of fast path; caller falls through to heuristic
              throw new Error('v9.21_block_header_safeClick_refused:' + _sc.reason);
            }
            if (!_sc) targetBlock.headerEl.click(); // safeClick not yet available (shouldn't happen)
          } catch (_) {}
          await sleep(1500);
          const urlAfter1 = W.urlNode();
          const turnAfter1 = W.urlTurn();
          const modalAfter1 = W.modalKind();
          if (window.__msTraceLog) window.__msTraceLog('post_stabilize_via_turn_card_block_click', {
            urlNode: urlAfter1, urlTurn: turnAfter1, modalKind: modalAfter1,
            click_idx: chainLen, click_player: targetPlayer,
          });
          if (urlAfter1 === ftTerminalNode && turnAfter1) {
            // Step 2: click the turn-card display on the board to open picker.
            // Trainer renders the turn card as <div class="relative cursor-pointer ...">
            // containing the rank text (e.g. "Q" for Qh). Match by first-rank char.
            let turnCardEl = null;
            try {
              const candidates = document.querySelectorAll('div.relative.cursor-pointer');
              for (const el of candidates) {
                const txt = (el.textContent || '').trim();
                if (txt && txt[0] === turnAfter1[0]) { turnCardEl = el; break; }
              }
            } catch (_) {}
            if (turnCardEl) {
              try {
                const _sc2 = W.safeClick ? await W.safeClick(turnCardEl, { purpose: 'v9.21_turn_card_click' }) : null;
                if (_sc2 && !_sc2.success) {
                  if (window.__msTraceLog) window.__msTraceLog('v9.21_turn_card_safeClick_refused', { reason: _sc2.reason });
                  throw new Error('v9.21_turn_card_safeClick_refused:' + _sc2.reason);
                }
                if (!_sc2) turnCardEl.click();
              } catch (_) {}
              await sleep(1500);
              const urlAfter2 = W.urlNode();
              const turnAfter2 = W.urlTurn();
              const modalAfter2 = W.modalKind();
              if (window.__msTraceLog) window.__msTraceLog('post_stabilize_via_turn_card_picker_open', {
                urlNode: urlAfter2, urlTurn: turnAfter2, modalKind: modalAfter2,
              });
              if (modalAfter2 === 'turn') {
                // v9.25: count this as a picker open (v9.21 fast path)
                try { window.__msPickerOpens = (window.__msPickerOpens||0) + 1; } catch (_) {}
                logEntry('stabilize_via_turn_card_success', {
                  chain_len: chainLen, click_idx: chainLen, click_player: targetPlayer,
                  after_block_click_url: urlAfter1,
                  after_picker_open_modal: modalAfter2,
                });
                if (window.__msTraceLog) window.__msTraceLog('post_stabilize_via_turn_card_success');
                return true;
              } else {
                if (window.__msTraceLog) window.__msTraceLog('post_stabilize_via_turn_card_bailed', {
                  reason: 'picker_did_not_open', modalKind: modalAfter2,
                });
                logEntry('stabilize_via_turn_card_bailed', {
                  reason: 'picker_did_not_open', modalKind: modalAfter2,
                });
                // fall through
              }
              ...
              if (window.__msTraceLog) window.__msTraceLog('post_stabilize_via_turn_card_bailed', {
                reason: 'turn_card_el_not_found',
              });
              logEntry('stabilize_via_turn_card_bailed', { reason: 'turn_card_el_not_found' });
              // fall through
            }
          } else {
            if (window.__msTraceLog) window.__msTraceLog('post_stabilize_via_turn_card_bailed', {
              reason: 'block_click_unexpected_state',
              urlNode: urlAfter1, urlTurn: turnAfter1,
              expected_urlNode: ftTerminalNode,
            });
            logEntry('stabilize_via_turn_card_bailed', {
              reason: 'block_click_unexpected_state',
              after_urlNode: urlAfter1, after_urlTurn: turnAfter1,
              expected_urlNode: ftTerminalNode,
            });
            // fall through
          }
          ...
          if (window.__msTraceLog) window.__msTraceLog('post_stabilize_via_turn_card_bailed', {
            reason: 'blocks_too_short_or_no_header',
            n_blocks: blocks0.length, chain_len: chainLen,
          });
          logEntry('stabilize_via_turn_card_bailed', {
            reason: 'blocks_too_short_or_no_header',
            n_blocks: blocks0.length, chain_len: chainLen,
    ...
    while (safety++ < 30) {
      ...
      const before = W.urlNode();
      const _targetIdx = blocks.indexOf(target);
      const _targetChosenLabel = target.actions.find(a => a.chosen)?.label || '?';
      if (window.__msTraceLog) window.__msTraceLog('pre_stabilize_click_player_' + (target.player || '?') + '_chosen_' + _targetChosenLabel, {
        click_idx: _targetIdx, click_player: target.player || null, click_chosen_label: _targetChosenLabel, before_urlNode: before,
      });
      target.headerEl.click();
      await sleep(2000);
      if (window.__msTraceLog) window.__msTraceLog('post_stabilize_click_attempt1', {
        before_urlNode: before, after_urlNode: W.urlNode(), after_urlTurn: W.urlTurn(),
      });
      ...
      if (W.urlNode() === before) {
        ...
        if (target2 && target2.headerEl) {
          const _t2Idx = blocks2.indexOf(target2);
          const _t2Chosen = target2.actions.find(a => a.chosen)?.label || '?';
          if (window.__msTraceLog) window.__msTraceLog('pre_stabilize_click_attempt2_player_' + (target2.player || '?'), {
            click_idx: _t2Idx, click_player: target2.player || null, click_chosen_label: _t2Chosen, before_urlNode: before,
          });
  ...
  window.__scrapeMultiStreet = async function(cfg) {
    // v9.23 (2026-05-27): normalize cfg.flop_terminal_filter -- accept
    //   function | array | string. Prior versions crashed with
    //   "cfg.flop_terminal_filter is not a function" when callers passed
    //   an array of terminal names (incident: plo5plomastermind1 on
    //   2026-05-27 used ["C-C"] form and aborted at 4s with zero nodes).
    try {
      if (cfg && cfg.flop_terminal_filter != null) {
        if (Array.isArray(cfg.flop_terminal_filter)) {
          const _filterArr = cfg.flop_terminal_filter.slice();
          const _filterSet = new Set(_filterArr);
          cfg.flop_terminal_filter = function (t) { return _filterSet.has(t); };
        } else if (typeof cfg.flop_terminal_filter === 'string') {
          const _filterStr = cfg.flop_terminal_filter;
          cfg.flop_terminal_filter = function (t) { return t === _filterStr; };
        } else if (typeof cfg.flop_terminal_filter !== 'function') {
          throw new Error('cfg.flop_terminal_filter must be function, array, or string -- got ' + typeof cfg.flop_terminal_filter);
        }
      }
    } catch (e) {
      console.error('[v9.23] flop_terminal_filter normalize failed:', e && e.message);
      throw e;
    }
    // v9.24 (2026-05-27): refuse to start if already in training mode.
    //   The user must navigate back to the normal Study mode first.
    try {
      if (window.__W && typeof window.__W.urlTrainingMode === 'function' && window.__W.urlTrainingMode()) {
        const mode = window.__W.urlTrainingMode();
        console.error('[v9.24] __scrapeMultiStreet refusing to start: trainingMode=' + mode + ' (url=' + location.href + ')');
        throw new Error('v9.24: cannot start scrape -- page is in training mode (' + mode + '). Navigate to Study mode first.');
      }
    } catch (e) {
      if (String(e.message).indexOf('v9.24') === 0) throw e;
      console.warn('[v9.24] training-mode pre-flight check threw (continuing):', e && e.message);
    }
    // v9.25 (2026-05-27): activate the nav guard CSS (disables pointer-events
    //   on top nav until session end). Initialize picker open/close counters.
    try {
      if (window.__W && typeof window.__W.activateNavGuard === 'function') {
        window.__W.activateNavGuard();
      }
      window.__msPickerOpens = 0;
      window.__msPickerCloses = 0;
    } catch (_) {}
    ...
    try {
      ...
      for (const ft of flopTerminals) {
        ...
        async function handleOneCell(targetCard, cellStatus) {
          // v9.25 diagnostic (2026-05-27): confirm orchestrator entry per card.
          // If this trace is missing for cards that nevertheless emitted zips,
          // the dfsStreet that's running comes from a different code path.
          try { window.__msTraceLog && window.__msTraceLog('handleOneCell_entry', { card: targetCard, status: cellStatus, ts: Date.now() }); } catch (_) {}
          if (window.__msQuotaExceeded || window.__msAborted) return 'break';

          if (explicitCards && !explicitCards.includes(targetCard)) {
            try { window.__msTraceLog && window.__msTraceLog('handleOneCell_skip_not_in_explicit', { card: targetCard }); } catch (_) {}
            return 'continue';
          }
        ...
        // v9.25 diagnostic: trace each cell-loop iteration
        let _cell_loop_iter_idx = 0;
        outer:
        while (cellSafety++ < CELL_SAFETY_MAX) {
          _cell_loop_iter_idx++;
          try { window.__msTraceLog && window.__msTraceLog('cell_loop_iter', { iter: _cell_loop_iter_idx, terminal: ft.terminal_node, modal: W.modalKind(), urlNode: W.urlNode(), urlTurn: W.urlTurn() }); } catch (_) {}
          if (window.__msQuotaExceeded || window.__msAborted) {
          ...
          try {
            ...
            if (pbChance > 0 && Math.random() < pbChance && typeof W.partialWalkOnTerminal === 'function') {
              // v9.26.6: how many partials per card? Default [1,1] = legacy behavior.
              let _pwLo = 1, _pwHi = 1;
              ...
              // v9.26.6: pass eligibility (workload candidate_pool + auto_aliased + dim_dom).
              const partialOpts = {
              ...
              for (let _pw = 0; _pw < _pwN; _pw++) {
                ...
                try {
                  ...
                  if (!_er.ok) result.warnings.push(`post-partial-walk cleanup at ${ft.terminal_node} (pw#${_pw+1}): ${_er.path}`);
                } catch (e) {
                  result.warnings.push(`post-partial-walk cleanup at ${ft.terminal_node} (pw#${_pw+1}): ${e.message}`);
                }
              }
              // legacy single-call line — kept commented for diff clarity:
  ...
  window.__msBuildSessionRecord = function(spec, cfg, result) {
    ...
    // v9.23 (2026-05-27): defensive backfill -- session_record was getting
    //   tree=null, flop=null, session_id='ses-unknown-<ts>' when the run
    //   aborted before result.tree/result.flop were populated (incident:
    //   plo5plomastermind1 on 2026-05-27). Backfill from cfg, then from the
    //   trainer URL via window.__W if still missing.
    const _W = window.__W || {};
    const _tree_resolved = (result && result.tree)
                          || (cfg && cfg.tree)
                          || (typeof _W.urlTree === 'function' ? _W.urlTree() : null)
                          || null;
    const _flop_resolved = (result && result.flop)
                          || (cfg && cfg.flop)
                          || (typeof _W.urlFlop === 'function' ? _W.urlFlop() : null)
                          || null;
    const _sid_resolved = (cfg && cfg.session_id)
                          || (cfg && cfg.session_id_base)
                          || ('ses-' + (_tree_resolved || 'unknown') + '-' + (_flop_resolved || 'noflop') + '-' + Date.now());
    const rec = {
      schema_version: '1',
    ...
    // v9.24 (2026-05-27): zip-count vs cards_planned sanity check.
    //   If turn_session walked fewer zips than planned cards, flag a warning
    //   so Phase E cleanup AND end-of-day report surface the gap.
    try {
      if (!isProducerMode && cards_planned_n > 0 && cards_walked_n < cards_planned_n) {
        rec.warnings = (rec.warnings || []).concat([
          'v9.24 zip-count mismatch: cards_planned=' + cards_planned_n +
          ' but cards_walked=' + cards_walked_n + ' (zips_emitted=' + zipNames.length + '). Possible early abort or training-mode incident.'
        ]);
      }
      // Also flag if exit_reason indicates training-mode abort
      if (window.__msEmergencyStop && (window.__W && window.__W.urlTrainingMode && window.__W.urlTrainingMode())) {
        rec.exit_reason = 'training_mode_entered';
        rec.warnings = (rec.warnings || []).concat([
          'v9.24: session aborted because page entered training mode mid-walk (URL: ' + location.href + ').'
        ]);
      }
    } catch (_) {}
    return rec;
  };
  // v9.25 (2026-05-27): hook to deactivate the nav guard. Called by
  //   __msBuildSessionRecord (which is the canonical "session ended" point
  //   in cron prompts) and also exposed directly so cron can invoke it
  //   independently if needed.
  window.__msDeactivateNavGuard = function() {
    try {
      if (window.__W && typeof window.__W.deactivateNavGuard === 'function') {
        window.__W.deactivateNavGuard();
      }
    } catch (_) {}
  };
  // Wrap __msBuildSessionRecord to auto-deactivate guard on completion.
  const _originalBuildSessionRecord = window.__msBuildSessionRecord;
  window.__msBuildSessionRecord = function(spec, cfg, result) {
    try {
      const rec = _originalBuildSessionRecord(spec, cfg, result);
      try { rec.picker_opens = window.__msPickerOpens || 0; } catch (_) {}
      try { rec.picker_closes = window.__msPickerCloses || 0; } catch (_) {}
      // Auto-deactivate the guard once the record is built
      try { window.__msDeactivateNavGuard(); } catch (_) {}
      return rec;
    } catch (e) {
      try { window.__msDeactivateNavGuard(); } catch (_) {}
      throw e;
