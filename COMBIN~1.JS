  ...
  async function dfsStreet(state, walkResult, opts = {}) {
    ...
    while (iterSafety++ < 12) {
      ...
      if (pickIsAllIn) {
        ...
        //   turn-just-dealt state at the flop terminal). Fires only on
        //   turn All-ins.
          ...
          try {
            ...
            if (typeof ftbIdx === 'number' && ftbIdx >= 0) {
              ...
              if (anchorBlock && anchorBlock.headerEl) {
                _msTraceLog('pre_v919_anchor_click_player_' + (anchorBlock.player || '?'), { click_idx: ftbIdx, click_player: anchorBlock.player || null, expect: state.node });
                const anchorResult = await clickHeaderAndWait(anchorBlock.headerEl, state.node);
                _msTraceLog('post_v919_anchor_click_' + (anchorResult.success ? 'ok' : 'mismatch') + '_after_' + anchorResult.after, { success: anchorResult.success, expect: state.node, after_observed: anchorResult.after });
                try {
