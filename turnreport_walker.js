(function () {
  function __TW_INSTALL() {
    ...
    // ---- QUOTA COUNTING (plo5-equivalent, adapted for a reload-heavy walker) ----
    // plo5 meters /range/url (n_range_url_calls). The REPORT scrapers never call it -
    // their metered endpoint is /agg-report. We count BOTH so range_url==0 proves the
    // reports are not eating the producer's quota.
    // We count from performance.getEntriesByType('resource'), NOT a fetch interceptor:
    // the walker does a full page RELOAD per node, which (a) wipes any window patch and
    // (b) fires the trainer's OWN page-load fetch before our code re-evals. The perf
    // timeline records every request on the page, including those we never saw.
    // Perf entries reset per page load -> key the read-mark on performance.timeOrigin.
    P.quota = function () { try { return JSON.parse(localStorage['__tw_quota']); } catch (e) {} return { session: '', started: new Date().toISOString(), agg_report: 0, range_url: 0, tree_config: 0, other_api: 0, s3_fetch: 0, http_429: 0, total_api: 0, _page: '', _mark: 0 }; };
    P.quotaReset = function (s) { const q = { session: s || '', started: new Date().toISOString(), agg_report: 0, range_url: 0, tree_config: 0, other_api: 0, s3_fetch: 0, http_429: 0, total_api: 0, _page: '', _mark: 0 }; localStorage['__tw_quota'] = JSON.stringify(q); return q; };
    P.quotaSnap = function () {
      const q = P.quota();
      const pk = String(Math.round(performance.timeOrigin || 0));
      if (q._page !== pk) { q._page = pk; q._mark = 0; }   // new page load -> perf list reset
      const ents = performance.getEntriesByType('resource');
      for (let i = q._mark || 0; i < ents.length; i++) {
        const u = ents[i].name || '';
        if (u.indexOf('execute-api') >= 0) {
          q.total_api++;
          if (u.indexOf('/agg-report') >= 0) q.agg_report++;
          else if (u.indexOf('/range/url') >= 0) q.range_url++;
          else if (u.indexOf('/postflop/tree/') >= 0) q.tree_config++;
          else q.other_api++;
        } else if (u.indexOf('amazonaws.com') >= 0) {
          q.s3_fetch++;   // presigned payload download - NOT a metered API call
        }
      }
      q._mark = ents.length;
      localStorage['__tw_quota'] = JSON.stringify(q);
      return q;
    };
    // 429 detection (best-effort, our own fetches; re-installed on every walker eval)
    if (!W.__twFetchPatched) { const _of = W.fetch; W.fetch = async function () { const r = await _of.apply(this, arguments); try { if (r && r.status === 429) { const q = P.quota(); q.http_429++; localStorage['__tw_quota'] = JSON.stringify(q); } } catch (e) {} return r; }; W.__twFetchPatched = 1; }
    ...
    // This fetch supplies ONLY the action-menu LABELS (the numbers come from React).
    // ROOT CAUSE of the old "col_mismatch" bug: `host` is derived from the performance
    // resource timeline, which can still be EMPTY immediately after a page reload -> host
    // = null -> malformed URL -> fetch fails. The previous retry re-used the SAME captured
    // ctx, so all 5 attempts hit the same null host and could never recover. Now we
    // RE-DERIVE the ctx on every attempt, so as soon as the app's first API call lands in
    // the timeline the retry succeeds. (Verified 2026-07-12: the endpoint itself returns
    // frames reliably for the nodes that were failing - it was never the server.)
    P._frames = async function (c, node, tries) {
      tries = tries || 5;
      for (let i = 0; i < tries; i++) {
        const cc = (c && c.host && c.auth) ? c : P._ctx();   // <- fresh ctx if incomplete
        if (cc && cc.host && cc.auth) {
          try {
            const r = await fetch(P._aggUrl(cc, node), { headers: { Authorization: cc.auth } });
    ...
    P._capObj = async function () { await P._ensureFzstd(); const list = JSON.parse(localStorage['__tw_list'] || '[]'); let c = P._ctx(); const node = c.node;
      let bds = null; for (let t = 0; t < 40; t++) { bds = P._boards(); if (bds && bds.length && bds[0].buckets) break; await sleep(500); }
      // RE-DERIVE after React is ready: the first _ctx() ran right after the reload,
      // when the performance timeline could still be empty (host = null).
      c = P._ctx();
      const status = { node }; if (!bds) { status.error = 'no report tree after wait'; }
      else { const frames = await P._frames(c, node); const cols = frames ? P._columns(frames) : []; const csv = P._csv(bds, cols, c.flop || ''); localStorage['__tw_csv_' + node] = csv; status.rows = csv.split('\n').length - 1; status.turn_cards = bds.length; status.cols = cols.map(x => x.label); status.col_mismatch = P._lastColInfo && P._lastColInfo.mismatch || false; if (!cols.length) status.no_menu = { host: !!c.host, auth: !!c.auth }; }
      P.quotaSnap();
      const next = list.find(x => localStorage['__tw_csv_' + x] === undefined); status.done = list.filter(x => localStorage['__tw_csv_' + x] !== undefined).length; status.total = list.length; status.next = next || null; status.quota = P.quota(); localStorage['__tw_last'] = JSON.stringify(status);
