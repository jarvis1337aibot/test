// walk_tree_to_zip.js - turn-tree Reports walker: one CSV per node -> one zip
// per terminal, human-paced. Config + ledger helpers for multi-flop auto mode.
// See templates/REPORTS_SESSION_PROMPT.md. Async returns are unreliable in cron,
// so status is mirrored to localStorage['__tw_last'] (read it via __TW.last(), sync).
(function () {
  function __TW_INSTALL() {
    const W = window; const P = (W.__TW = W.__TW || {});
    const sleep = ms => new Promise(r => setTimeout(r, ms));
    P.LEDGER_KEY = 'plo_turnreport_ledger';
    // ---- human timing profile (persisted across reloads) ----
    ...
    // ---- auth + host. Host from ANY execute-api URL (regex to /prod), so it works
    // on the picker/home page too, not just after an agg-report. Token from
    // localStorage['plo-auth-5c'] (base64 JSON -> accessToken) + PREPEND Bearer.
    ...
    // size -> label. Common pot fractions named; anything else falls back to
    // "<n>% pot" so ANY tree's sizings read/write correctly (never crashes).
    ...
    P._frames = async function (c, node) { try { const j = await fetch(P._aggUrl(c, node), { headers: { Authorization: c.auth } }).then(r => r.json()); return (j && j.frames) ? j.frames : null; } catch (e) { return null; } };
    // Column defs from the UNION of every card's binary action menu, canonical
    // order (Fold, Check/Call, raises ASCENDING by amount, All-in). Aligns 1:1
    // with the React `values` slots -> correct labels for any tree's sizings.
    ...
    // ---- tree config -> flops + terminals (turn roots). AUTHORITATIVE per tree:
    // terminal COUNT + sizings vary by tree/flop; always read live, never hardcode.
    ...
    // ---- ledger: separate key, structured tree -> flop -> terminal -> {status,...}
    ...
    // Reconcile label count (binary menu union) with React values length (the true
    // number of data columns). Use the larger; label any extra slots generically;
    // record a mismatch flag so the caller can surface it.
